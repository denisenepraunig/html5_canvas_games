...
$(document).ready(function documentReady() {
	 ...
	 * @property {Object} defaults.player.speed.default
	...
	function Player(x, y, w, h, color) {
		...
		var speed = {
			vX : defaults.player.speed.default,
			vY : defaults.player.speed.default
		};
	...
	/**
	 * KeyDown Listener for keyboard control
	...
	function onKeyDown(e) {
		...
		if      (kc === 37) move.left = true;  // only one key per event
		else if (kc === 38) move.up = true;    // so check exclusively
		else if (kc === 39) move.right = true;
		else if (kc === 40) move.down = true;
	}

	/**
	 * KeyUp Listener for keyboard events
	 * @param {Object} e - the keyboard event
	 */
	function onKeyUp(e) {
		...
		if      (kc === 37) move.left = false;
		else if (kc === 38) move.up = false;
		else if (kc === 39) move.right = false;
		else if (kc === 40) move.down = false;
	}
	...
	function updatePlayer() {
		...
		else if (move.down) {  // both up and down does not work so check excl.
			player.y += player.vY * deltaFactor;
		}
